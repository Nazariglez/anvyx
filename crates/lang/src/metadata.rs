...
pub fn exports_to_json(decls: &[ExternDecl]) -> String {
    let mut out = String::from("{\"types\":[],\"functions\":[");
...
pub fn parse_provider_json(json: &str) -> Result<ExternProviderMeta, String> {
    ...
    Ok(ExternProviderMeta { functions: funcs })
}
...
pub(crate) fn anvyx_type_from_str(s: &str) -> Result<crate::ast::Type, String> {
    match s {
        ...
        other => Err(format!("Unknown Anvyx type in extern metadata: '{other}'")),
    }
}
...
pub(crate) fn metadata_to_extern_stmts(
    ...
    use crate::ast::{ExternFunc, Ident, Mutability, Param, Stmt};
...
mod tests {
    ...
    fn anvyx_type_from_str_unknown() {
        let result = anvyx_type_from_str("widget");
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("widget"));
    }
